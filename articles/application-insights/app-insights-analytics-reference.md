...
# <a name="reference-for-analytics"></a>Reference for Analytics
...
## <a name="queries-and-operators"></a>Queries and operators
...
* `bagexpansion=array`: Property bags are expanded into two-element `[`*key*`,`*value*`]` array structures, allowing uniform access to keys and values (as well as, for example, running a distinct-count aggregation over property names). 
...
The following example shows several kinds of manipulations that can be done using the `project` operator. The input table `T` has three columns of type `int`: `A`, `B`, and `C`. 
...
A table with a single column called *ColumnName*, whose values are *Start*, *Start* + *Step*, ... up to and including *Stop*.
...
A table with a single column called `Steps` whose type is `long` and whose values are `1`, `4`, and `7`.
...
Shows how the `range` operator can be used to create a small, ad-hoc, dimension table which is then used to introduce zeros where the source data has no values.
...
### <a name="render-directive"></a>render directive
...
### <a name="restrict-clause"></a>restrict clause 
...
* `withsource=`*ColumnName:* If specified, the output will include a column called *ColumnName* whose value indicates which source table has contributed each row.
...
```
The number of distinct users that have produced either a `exceptions` event or a `traces` event over the past day. In the result, the 'SourceTable' column will indicate either "Query" or "Command".
...
Records that are no older than 1 hour, and come from the Source called "Kuskus", and have two columns of the same value. 
...
#### <a name="weighted-percentiles"></a>Weighted percentiles
...
#### <a name="estimation-error-in-percentiles"></a>Estimation error in percentiles
...
### <a name="scalar-comparisons"></a>Scalar comparisons
...
The `iff()` function evaluates the first argument (the predicate), and returns either the value of either the second or third arguments depending on whether the predicate is `true` or `false`. The second and third arguments must be of the same type.
...
### <a name="boolean-literals"></a>Boolean Literals
...
### <a name="arithmetic-operators"></a>Arithmetic operators
...
## <a name="date-and-time"></a>Date and time


[ago](#ago) | [dayofmonth](#dayofmonth) | [dayofweek](#dayofweek) |  [dayofyear](#dayofyear) |[datepart](#datepart) | [endofday](#endofday) | [endofmonth](#endofmonth) | [endofweek](#endofweek) | [endofyear](#endofyear) | [getmonth](#getmonth)|  [getyear](#getyear) | [now](#now) | [startofday](#startofday) | [startofmonth](#startofmonth) | [startofweek](#startofweek) | [startofyear](#startofyear) | [todatetime](#todatetime) | [totimespan](#totimespan) | [weekofyear](#weekofyear)

### <a name="date-and-time-literals"></a>Date and time literals
...
### <a name="date-and-time-expressions"></a>Date and time expressions
...
Subtracts the given timespan from the current UTC clock time. Like `now()`, this function can be used multiple times in a statement and the UTC clock time being referenced will be the same for all instantiations.
...
* *a_timespan*: Interval to subtract from the current UTC clock time (`now()`).
...
### <a name="obfuscated-string-literals"></a>Obfuscated String Literals
...
## <a name="arrays-objects-and-dynamic"></a>Arrays, objects and dynamic
...
### <a name="array-and-object-literals"></a>Array and object literals
...
### <a name="dynamic-object-functions"></a>Dynamic object functions
...
### <a name="dynamic-objects-in-let-clauses"></a>Dynamic objects in let clauses
...
### <a name="json-path-expressions"></a>JSON Path expressions
...
The `range()` function (not to be confused with the `range` operator) generates a dynamic array holding a series of equally-spaced values.
...
* *step:* The difference between two consecutive elements of the array.
