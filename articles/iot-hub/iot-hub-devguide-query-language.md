...
# <a name="reference---query-language-for-twins-and-jobs"></a>Reference - query language for twins and jobs
...
## <a name="getting-started-with-twin-queries"></a>Getting started with twin queries
...
### <a name="node-example"></a>Node example
...
## <a name="getting-started-with-jobs-queries"></a>Getting started with jobs queries
...
## <a name="basics-of-an-iot-hub-query"></a>Basics of an IoT Hub query
...
The SELECT clause (**SELECT <select_list>**) is mandatory and specifies what values will be retrieved from the query. It specifies the JSON values to be used to generate new JSON objects For each element of the filtered (and optionally grouped) subset of the FROM collection, the projection phase generates a new JSON object, constructed with the values specified in the SELECT clause.
...
## <a name="expressions-and-conditions"></a>Expressions and conditions
