...
# <a name="use-custom-activities-in-an-azure-data-factory-pipeline"></a>Use custom activities in an Azure Data Factory pipeline
> [AZURE.SELECTOR]
...
> [AZURE.IMPORTANT] Set 4.5.2 version of .NET Framework as the target framework for your .NET custom activity project in Visual Studio. Data Factory does not support custom activities compiled against .NET Framework versions later than 4.5.2.   
...
### <a name="azure-batch-prerequisites"></a>Azure Batch prerequisites
...
## <a name="create-the-custom-activity"></a>Create the custom activity
...
10. Compile the project. Click **Build** from the menu and click **Build Solution**.
    ...
    > [AZURE.IMPORTANT] Set 4.5.2 version of .NET Framework as the target framework for your project: right-click the project, and click **Properties** to set the target framework. Data Factory does not support custom activities compiled against .NET Framework versions later than 4.5.2. 
...
### <a name="execute-method"></a>Execute method
...
## <a name="create-the-data-factory-using-azure-portal"></a>Create the data factory using Azure portal
...
### <a name="step-1-create-the-data-factory"></a>Step 1: Create the data factory
...
### <a name="step-2-create-linked-services"></a>Step 2: Create linked services
...
#### <a name="create-azure-batch-linked-service"></a>Create Azure Batch linked service
...
### <a name="step-3-create-datasets"></a>Step 3: Create datasets
...
### <a name="create-and-run-a-pipeline-that-uses-the-custom-activity"></a>Create and run a pipeline that uses the custom activity
...
10. You should see that the five output slices are in the Ready state if they have already been produced.
...
9. If you open the output file, you should see the output similar to the following output:
...
### <a name="data-factory-and-batch-integration"></a>Data Factory and Batch integration
...
## <a name="debug-the-pipeline"></a>Debug the pipeline
...
7.  The custom activity does not use the **app.config** file from your package, so if your code reads any connection strings from the configuration file, it does not work at runtime. The best practice when using Azure Batch is to hold any secrets in an **Azure KeyVault**, use a certificate-based service principal to protect the **keyvault**, and distribute the certificate to Azure Batch pool. The .NET custom activity then can access secrets from the KeyVault at runtime. This solution is a generic solution and can scale to any type of secret, not just connection string.
...
## <a name="update-the-custom-activity"></a>Update the custom activity
...
## <a name="copymove-data"></a>Copy/Move data 
...
## <a name="access-extended-properties"></a>Access extended properties
...
## <a name="auto-scaling-feature-of-azure-batch"></a>Auto-scaling feature of Azure Batch
...
## <a name="use-azure-hdinsight-linked-services"></a>Use Azure HDInsight linked services
...
##### <a name="to-use-an-on-demand-hdinsight-cluster"></a>To use an on-demand HDInsight cluster
...
##### <a name="to-use-your-own-hdinsight-cluster"></a>To use your own HDInsight cluster:
