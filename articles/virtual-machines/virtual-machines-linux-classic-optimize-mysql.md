...
#<a name="optimizing-mysql-performance-on-azure-linux-vms"></a>Optimizing MySQL Performance on Azure Linux VMs
...
##<a name="utilizing-raid-on-an-azure-virtual-machine"></a>Utilizing RAID on an Azure virtual machine
...
###<a name="setting-up-raid-on-azure"></a>Setting up RAID on Azure
...
####<a name="step-1-add-a-data-disk-to-your-virtual-machine"></a>Step 1: Add a Data Disk to your Virtual Machine  
...
####<a name="step-2-create-raid-with-the-additional-disks"></a>Step 2: Create RAID with the additional disks
...
####<a name="step-3-set-up-a-new-storage-path"></a>Step 3: Set up a new storage path
Use the following command:  

    root@mysqlnode1:~# mkdir -p /RAID0/mysql

####<a name="step-4-copy-the-original-data-to-the-new-storage-path"></a>Step 4: Copy the original data to the new storage path
Use the following command:  

    root@mysqlnode1:~# cp -rp /var/lib/mysql/* /RAID0/mysql/

####<a name="step-5-modify-permissions-so-mysql-can-access-read-and-write-the-data-disk"></a>Step 5: Modify permissions so MySQL can access (read and write) the data disk
Use the following command:  

    root@mysqlnode1:~# chown -R mysql.mysql /RAID0/mysql && chmod -R 755 /RAID0/mysql


##<a name="adjust-the-disk-io-scheduling-algorithm"></a>Adjust the disk I/O scheduling algorithm
...
###<a name="step-1view-the-current-io-scheduler"></a>Step 1.View the current I/O scheduler
Use the following command:  
...
###<a name="step-2-change-the-current-device-devsda-of-io-scheduling-algorithm"></a>Step 2. Change the current device (/dev/sda) of I/O scheduling algorithm
...
##<a name="configure-system-file-operations-settings"></a>Configure system file operations settings
...
##<a name="increase-the-maximum-number-of-system-handles-for-high-concurrency"></a>Increase the maximum number of system handles for high concurrency
...
###<a name="step-1-modify-the-limitsconf-file"></a>Step 1: Modify the limits.conf file
...
###<a name="step-2-update-the-system-for-the-new-limits"></a>Step 2: Update the system for the new limits
Run the following commands:  

    ulimit -SHn 65536
    ulimit -SHu 65536

###<a name="step-3-ensure-that-the-limits-are-updated-at-boot-time"></a>Step 3: Ensure that the limits are updated at boot time
Put the following startup commands in the /etc/rc.local file so it will take effect during every boot time.  

    echo “ulimit -SHn 65536” >>/etc/rc.local
    echo “ulimit -SHu 65536” >>/etc/rc.local

##<a name="mysql-database-optimization"></a>MySQL database optimization
...
##<a name="turn-on-the-mysql-slow-query-log-for-analyzing-the-performance-bottleneck"></a>Turn on the MySQL slow query log for analyzing the performance bottleneck
...
###<a name="step-1-modify-mycnf-file-by-adding-the-following-lines-to-the-end"></a>Step 1: Modify my.cnf file by adding the following lines to the end   

    long_query_time = 2
    slow_query_log = 1
    slow_query_log_file = /RAID0/mysql/mysql-slow.log

###<a name="step-2-restart-mysql-server"></a>Step 2: Restart mysql server
    service  mysql  restart

###<a name="step-3-check-whether-the-setting-is-taking-effect-using-the-show-command"></a>Step 3: Check whether the setting is taking effect using the “show” command
