...
# <a name="running-cassandra-with-linux-on-azure-and-accessing-it-from-nodejs"></a>Running Cassandra with Linux on Azure and Accessing it from Node.js
...
## <a name="the-deployment-models"></a>The Deployment Models
...
### <a name="single-region-deployment"></a>Single Region Deployment
...
### <a name="data-consistency-for-multi-data-center-deployment"></a>Data Consistency for Multi-Data Center Deployment
...
##<a name="the-software-configuration"></a>THE SOFTWARE CONFIGURATION
...
####<a name="step-1-generate-ssh-key-pair"></a>STEP 1: Generate SSH key pair
...
####<a name="step-2-create-ubuntu-template-vm"></a>STEP 2: Create Ubuntu template VM
...
###<a name="install-the-necessary-software"></a>INSTALL THE NECESSARY SOFTWARE
####<a name="step-1-upload-tarballs"></a>STEP 1: Upload tarballs
...
####<a name="step-2-prepare-the-directory-structure-and-extract-the-archives"></a>STEP 2: Prepare the directory structure and extract the archives
...
####<a name="step-3-edit-etcprofile"></a>Step 3: Edit etc/profile
...
####<a name="step-4-install-jna-for-production-systems"></a>Step 4: Install JNA for production systems
Use the following command sequence: The following command will install jna-3.2.7.jar and jna-platform-3.2.7.jar to /usr/share.java directory sudo apt-get install libjna-java
...
####<a name="step-5-configure-cassandrayaml"></a>Step 5: Configure cassandra.yaml
...
####<a name="step-6-capture-the-vm-image"></a>Step 6: Capture the VM image
...
##<a name="single-region-deployment-process"></a>Single Region Deployment Process
**Step 1: Create the Virtual Network** Log into the Azure classic portal and create a Virtual Network with the attributes show in the table. See [Configure a Cloud-Only Virtual Network in the Azure classic portal](../virtual-network/virtual-networks-create-vnet-classic-portal.md) for detailed steps of the process.      
...
**Step 2: Provision Virtual Machines** Using the image created previously, we will create the following virtual machines in the cloud server “hk-c-svc-west” and bind them to the respective subnets as shown below:
...
## <a name="test-the-single-region-cluster"></a>Test the Single Region Cluster
...
###<a name="step-1-create-the-virtual-network-at-the-2nd-region"></a>Step 1: Create the Virtual Network at the 2nd Region
...
###<a name="step-2-create-local-networks"></a>Step 2: Create Local Networks
...
###<a name="step-3-map-local-network-to-the-respective-vnets"></a>Step 3: Map “Local” network to the respective VNETs
...
###<a name="step-4-create-gateways-on-vnet1-and-vnet2"></a>Step 4: Create Gateways on VNET1 and VNET2
...
###<a name="step-5-update-local-networks-with-the-respective-gateway-addresses"></a>Step 5: Update “Local” networks with the respective “Gateway” addresses###
...
###<a name="step-6-update-the-shared-key"></a>Step 6: Update the shared key
...
###<a name="step-7-establish-the-vnet-to-vnet-connection"></a>Step 7: Establish the VNET-to-VNET connection
...
###<a name="step-8-create-the-virtual-machines-in-region-2"></a>Step 8: Create the virtual machines in region #2
...
###<a name="step-9-configure-cassandra-on-each-vm"></a>Step 9: Configure Cassandra on each VM
Log into the VM and perform the following:
...
2. Edit cassandra.yaml to configure seed nodes:  Seeds: "10.1.2.4,10.1.2.6,10.1.2.8,10.1.2.10,10.2.2.4,10.2.2.6,10.2.2.8,10.2.2.10"

###<a name="step-10-start-cassandra"></a>Step 10: Start Cassandra
Log into each VM and start Cassandra in the background by running the following command: $CASS_HOME/bin/cassandra

## <a name="test-the-multi-region-cluster"></a>Test the Multi-Region Cluster
...
###<a name="step-1-get-the-internal-load-balancer-ip-for-both-the-regions-using-powershell"></a>Step 1: Get the internal load balancer IP for both the regions using PowerShell
- Get-AzureInternalLoadbalancer -ServiceName "hk-c-svc-west-us"
- Get-AzureInternalLoadbalancer -ServiceName "hk-c-svc-east-us"  

    Note the IP addresses (e.g. west - 10.1.2.101, east - 10.2.2.101) displayed.

###<a name="step-2-execute-the-following-in-the-west-region-after-logging-into-hk-w1-west-us"></a>Step 2: Execute the following in the west region after logging into hk-w1-west-us
...
###<a name="step-3-execute-the-following-in-the-east-region-after-logging-into-hk-w1-east-us"></a>Step 3: Execute the following in the east region after logging into hk-w1-east-us:
...
## <a name="test-cassandra-cluster-from-nodejs"></a>Test Cassandra Cluster from Node.js
