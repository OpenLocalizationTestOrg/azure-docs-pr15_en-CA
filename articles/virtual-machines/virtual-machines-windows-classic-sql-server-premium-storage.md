...
# <a name="use-azure-premium-storage-with-sql-server-on-virtual-machines"></a>Use Azure Premium Storage with SQL Server on Virtual Machines
...
**Author:** Daniel Sol **Technical Reviewers:** Luis Carlos Vargas Herring, Sanjay Mishra, Pravin Mital, Juergen Thomas, Gonzalo Ruiz.
...
### <a name="machine-size"></a>Machine size
...
#### <a name="how-to-find-which-azure-virtual-disks-map-to-storage-pools"></a>How to find which Azure Virtual Disks map to storage pools
...
### <a name="vm-storage-bandwidth-and-vhd-storage-throughput"></a>VM storage bandwidth and VHD storage throughput
...
## <a name="new-deployments"></a>New deployments
...
### <a name="create-a-new-vm-with-premium-storage-with-gallery-image"></a>Create a new VM with Premium Storage with Gallery Image
...
#### <a name="step-1-create-a-premium-storage-account"></a>Step 1: Create a Premium Storage Account
...
#### <a name="step-2-create-a-new-cloud-service"></a>Step 2: Create a New Cloud Service

    $destcloudsvc = "danNewSvcAms"
    New-AzureService $destcloudsvc -Location $location


#### <a name="step-3-reserve-a-cloud-service-vip-optional"></a>Step 3: Reserve a Cloud Service VIP (Optional)
...
#### <a name="step-4-create-a-vm-container"></a>Step 4: Create a VM Container
...
#### <a name="step-5-placing-os-vhd-on-standard-or-premium-storage"></a>Step 5: Placing OS VHD on Standard or Premium Storage
...
#### <a name="step-6-create-vm"></a>Step 6: Create VM
...
### <a name="create-a-new-vm-to-use-premium-storage-with-a-custom-image"></a>Create a new VM to use Premium Storage with a custom image
...
#### <a name="step-1-create-storage-account"></a>Step 1: Create Storage Account
    $mysubscription = "DansSubscription"
    $location = "West Europe"
...
#### <a name="step-2-create-cloud-service"></a>Step 2 Create Cloud Service
    $destcloudsvc = "danNewSvcAms"
    New-AzureService $destcloudsvc -Location $location


#### <a name="step-3-use-existing-image"></a>Step 3: Use existing image
...
#### <a name="step-4-copy-blob-between-storage-accounts"></a>Step 4: Copy Blob between Storage Accounts
    #Get Image VHD
...
#### <a name="step-5-regularly-check-copy-status"></a>Step 5: Regularly check copy status:
    $blob | Get-AzureStorageBlobCopyState

#### <a name="step-6-add-image-disk-to-azure-disk-repository-in-subscription"></a>Step 6: Add Image disk to Azure disk Repository in Subscription
...
#### <a name="step-7--build-the-vm"></a>Step 7:  Build the VM
...
## <a name="existing-deployments-that-do-not-use-always-on-availability-groups"></a>Existing deployments that do not use Always On Availability Groups
...
## <a name="existing-deployments-that-use-always-on-availability-groups"></a>Existing deployments that use Always On Availability Groups
...
### <a name="migrating-always-on-deployments-that-can-allow-some-downtime"></a>Migrating Always On deployments that can allow some downtime
...
#### <a name="1-add-more-secondary-replicas-to-an-existing-always-on-cluster"></a>1. Add more Secondary Replicas to an Existing Always On Cluster
...
##### <a name="points-of-downtime"></a>Points of downtime:
...
#### <a name="2-migrate-to-a-new-always-on-cluster"></a>2. Migrate to a new Always On Cluster
...
### <a name="migrating-always-on-deployments-for-minimal-downtime"></a>Migrating Always On Deployments for minimal downtime
...
#### <a name="1-utilize-an-existing-secondary-single-site"></a>1. Utilize an existing secondary: Single-Site
...
#### <a name="2-utilize-existing-secondary-replicas-multi-site"></a>2. Utilize existing secondary replica(s): Multi-Site
...
## <a name="appendix-migrating-a-multisite-always-on-cluster-to-premium-storage"></a>Appendix: Migrating a Multisite Always On Cluster to Premium Storage
...
### <a name="pre-steps-connect-to-subscription"></a>Pre Steps: Connect to Subscription
...
#### <a name="step-1-create-new-storage-account-and-cloud-service"></a>Step 1: Create New Storage Account and Cloud Service
    $mysubscription = "DansSubscription"
    $location = "West Europe"
...
#### <a name="step-2-increase-the-permitted-failures-on-resources-optional"></a>Step 2: Increase the permitted failures on resources<Optional>
...
#### <a name="step-3-addition-ip-address-resource-for-cluster-group-optional"></a>Step 3: Addition IP Address resource for Cluster Group<Optional>
...
#### <a name="step-4-dns-configuration"></a>Step 4: DNS configuration
...
##### <a name="client-application-settings"></a>Client application settings
...
#### <a name="step-5-cluster-quorum-settings"></a>Step 5: Cluster quorum settings
...
#### <a name="step-6-extract-existing-endpoints-and-acls"></a>Step 6: Extract Existing Endpoints and ACLs
    #GET Endpoint info
...
#### <a name="step-7-change-failover-partners-and-replication-modes"></a>Step 7: Change Failover Partners and Replication Modes
...
#### <a name="step-8-remove-secondary-vm-from-cloud-service"></a>Step 8: Remove Secondary VM from cloud service
...
#### <a name="step-9-change-disk-caching-settings-in-csv-file-and-save"></a>Step 9: Change disk caching settings in CSV file and save
...
#### <a name="step-10-copy-vhds"></a>Step 10: Copy VHDS
...
#### <a name="step-11-register-os-disk"></a>Step 11: Register OS disk
...
#### <a name="step-12-import-secondary-into-new-cloud-service"></a>Step 12: Import secondary into new cloud service
...
#### <a name="step-13-create-ilb-on-new-cloud-svc-add-load-balanced-endpoints-and-acls"></a>Step 13: Create ILB on New Cloud Svc, Add Load Balanced Endpoints and ACLs
...
####<a name="step-14-update-always-on"></a>Step 14: Update Always On
...
#### <a name="step-15-dns-update-check"></a>Step 15: DNS update check
...
#### <a name="step-16-reconfigure-always-on"></a>Step 16: Reconfigure Always On
...
#### <a name="step-17-migrate-second-node"></a>Step 17: Migrate second node
...
#### <a name="step-18-change-disk-caching-settings-in-csv-file-and-save"></a>Step 18: Change disk caching settings in CSV file and save
...
#### <a name="step-19-create-new-independent-storage-account-for-secondary-node"></a>Step 19: Create New Independent Storage Account for Secondary Node
...
#### <a name="step-20-copy-vhds"></a>Step 20: Copy VHDS
    #Ensure you have created the container for these:
    $containerName = 'vhds'
...
You can check the VHD copy status for all VHDs: ForEach ($disk in $diskobjects) { $lun = $disk. Lun $vhdname = $disk.vhdname $cacheoption = $disk. HostCaching $disklabel = $disk. DiskLabel $diskName = $disk. DiskName
...
For information for individual blobs: #Check induvidual blob status Get-AzureStorageBlobCopyState -Blob "danRegSvcAms-dansqlams1-2014-07-03.vhd" -Container $containerName -Context $xioContextnode2

#### <a name="step-21-register-os-disk"></a>Step 21: Register OS disk
...
#### <a name="step-22-add-load-balanced-endpoints-and-acls"></a>Step 22: Add Load Balanced Endpoints and ACLs
    #Endpoints
    $epname="sqlIntEP"
    $prot="tcp"
    $locport=1433
...
#### <a name="step-23-test-failover"></a>Step 23: Test failover
...
#### <a name="step-24-put-back-cluster-quorum-settings--dns-ttl--failover-pntrs--sync-settings"></a>Step 24: Put back cluster quorum settings / DNS TTL / Failover Pntrs / Sync Settings
##### <a name="adding-ip-address-resource-on-same-subnet"></a>Adding IP Address Resource on Same Subnet
