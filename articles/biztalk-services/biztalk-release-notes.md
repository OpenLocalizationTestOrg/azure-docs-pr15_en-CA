...
## <a name="update-history"></a>Update History

### <a name="october-update"></a>October Update
...
## <a name="know-issues"></a>Know Issues

### <a name="connectivity-issues-after-biztalk-services-portal-update"></a>Connectivity Issues after BizTalk Services Portal Update
...
### <a name="visual-studio-ide-cannot-locate-the-artifact-if-you-click-an-error-or-warning-in-a-biztalk-services-project"></a>Visual Studio IDE cannot locate the artifact if you click an error or warning in a BizTalk Services project
Install the Visual Studio 2012 Update 3 RC 1 to fix the issue.  

### <a name="custom-binding-project-reference"></a>Custom binding project reference
...
### <a name="mdn-is-sent-asynchronously-even-if-the-send-asynchronous-mdn-option-is-unchecked"></a>MDN is sent asynchronously even if the Send asynchronous MDN option is unchecked  
...
### <a name="whitespace-characters-beyond-a-valid-interchange-cause-an-empty-message-to-be-sent-to-the-suspend-endpoint"></a>Whitespace characters beyond a valid interchange cause an empty message to be sent to the suspend endpoint  
...
### <a name="tracking-in-biztalk-services-portal"></a>Tracking in BizTalk Services Portal  
...
### <a name="update-agreement"></a>Update Agreement  
...
### <a name="as2-attachments"></a>AS2 Attachments  
...
### <a name="resources-remembering-path"></a>Resources: Remembering Path  
...
### <a name="if-you-rename-the-entity-name-of-a-bridge-and-close-the-project-without-saving-changes-opening-the-entity-again-results-in-an-error"></a>If you rename the entity name of a bridge and close the project without saving changes, opening the entity again results in an error
...
### <a name="biztalk-service-project-builds-successfully-even-if-an-artifact-has-been-excluded-from-a-visual-studio-project"></a>BizTalk Service project builds successfully even if an artifact has been excluded from a Visual Studio project
...
### <a name="the-biztalk-service-project-does-not-check-for-schema-availability-while-configuring-the-bridges"></a>The BizTalk Service project does not check for schema availability while configuring the bridges
...
### <a name="the-response-message-for-a-xml-request-reply-bridge-is-always-of-charset-utf-8"></a>The response message for a XML Request-Reply Bridge is always of charset UTF-8
For this release, the charset of the response message from an XML Request-Reply Bridge is always set to UTF-8.
### <a name="user-defined-datatypes"></a>User-Defined Datatypes
...
### <a name="special-characters-should-not-be-used-for-address-and-entity-names-of-lob-components"></a>Special characters should not be used for address and entity names of LOB components
...
### <a name="test-map-with-get-context-property"></a>Test Map with Get Context Property
...
### <a name="test-map-property-does-not-display"></a>Test Map Property does not display
...
### <a name="datetime-reformat-drop-down-is-grayed-out"></a>DateTime Reformat drop-down is grayed out
...
### <a name="duplicate-agreements-in-the-biztalk-services-portal"></a>Duplicate agreements in the BizTalk Services Portal
...
### <a name="bridges-do-not-use-updated-certificate-even-after-a-certificate-has-been-updated-in-the-artifact-store"></a>Bridges do not use updated certificate even after a certificate has been updated in the artifact store
...
### <a name="bridges-continue-to-process-messages-even-when-the-sql-database-is-offline"></a>Bridges continue to process messages even when the SQL database is offline
...
### <a name="reading-the-xml-message-within-a-bridges-custom-code-component-includes-an-extra-bom-character"></a>Reading the XML message within a bridge’s custom code component includes an extra BOM character
...
### <a name="sending-messages-to-a-bridge-using-wcf-does-not-scale"></a>Sending messages to a bridge using WCF does not scale
Messages sent to a bridge using WCF does not scale. You should instead use HttpWebRequest if you want a scalable client.
### <a name="upgrade-token-provider-error-after-upgrading-from-biztalk-services-preview-to-general-availability-ga"></a>UPGRADE: Token Provider error after upgrading from BizTalk Services Preview to General Availability (GA)
...
### <a name="upgrade-toolbox-shows-the-old-bridge-icons-after-upgrading-the-biztalk-services-sdk"></a>UPGRADE: Toolbox shows the old bridge icons after upgrading the BizTalk Services SDK
...
### <a name="upgrade-biztalk-portal-update-from-preview-to-ga-might-show-an-error-indicating-that-the-edi-capability-is-not-available"></a>UPGRADE: BizTalk Portal update from Preview to GA might show an error indicating that the EDI capability is not available
...
### <a name="upgrade-new-tracking-data-does-not-show-up-after-biztalk-services-is-upgraded-to-ga"></a>UPGRADE: New tracking data does not show up after BizTalk Services is upgraded to GA
...
### <a name="pipelines-vs-bridges"></a>Pipelines v/s Bridges
